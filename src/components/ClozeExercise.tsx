...
const CASE_COLORS: Record<string, string> = {
  Nominativ: 'bg-blue-100 text-blue-700',
  Akkusativ: 'bg-orange-100 text-orange-700',
  Dativ: 'bg-purple-100 text-purple-700',
  Genitiv: 'bg-rose-100 text-rose-700',
};
...
function renderParagraph(
  ...
  return parts.map((part, i) => {
    ...
    const info = placeholderMap.get(part);
    if (!info) return <span key={i}>{part}</span>;

    const { questionId, choices } = info;
    const selected = answers[questionId];
    const fb = feedback?.find(f => f.questionId === questionId) ?? null;
...
function InlineSelect({ choices, selected, feedback, onSelect, disabled }: InlineSelectProps) {
  ...
  const icon = feedback ? (feedback.correct ? '✓' : '✗') : null;
...
export default function ClozeExercise({ topicLabel, texts, onFinish, onHome }: Props) {
  ...
  const scoreColor =
    !feedback ? '' :
    score / totalQuestions >= 0.7 ? 'bg-green-100 text-green-700' :
    score / totalQuestions >= 0.5 ? 'bg-yellow-100 text-yellow-700' :
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="text-center">
            <p className="text-xs text-gray-400 font-medium uppercase tracking-wide">{topicLabel}</p>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    <span className={`ml-auto text-xs px-2 py-0.5 rounded-full font-medium ${CASE_COLORS[fb.grammarCase] ?? 'bg-gray-100 text-gray-700'}`}>
                      {fb.grammarCase}
                    </span>
                  </div>
                  {!fb.correct && (
                  ...
                  {!fb.correct && fb.nounNote && (
                    <p className="text-xs text-indigo-700 mt-1 ml-6 bg-indigo-50 rounded-lg px-2 py-1">
                      📖 {fb.nounNote}
                    </p>
                  )}
                </div>
